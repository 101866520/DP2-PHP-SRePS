...
namespace PHP_SRePS
{
    ...
    public class Prediction
    {
        ...
        private decimal AvgSale(List<Item> itemList)
        {
        ...
        public List<ReportItem> GenerateReport(List<Item> itemList)
        {
            ...
            foreach (Item item in itemList)
            {
                ...
                report.TotalSale = TotalSale(itemList);
                report.AvgSale = AvgSale(itemList);
                Tuple<decimal, decimal> temp = TrendLine(itemList);
                report.PredictedSaleWeek = PredictingSale(itemList, temp, 7);
                report.PredictedSaleMonth = PredictingSale(itemList, temp, 30);
                if (temp.Item1 > 0)
                {
                    report.Trend = "Up";
                }
